...
const DepartmentPlanningPolicyPage = () => {
  ...
  useEffect(() => {
    const fetchPageContent = async () => {
      try {
        console.log("Fetching page with slug:", 'planning-policy-department');
        const response = await fetch(GET_PAGE_BY_SLUG_PUBLI_URL('planning-policy-department'), {
  ...
  if (loading) {
    return (
      <PublicLayout>
        <PageHeader pageName="Planning Policy and Coordination Department" />
  ...
  const departmentSection1 = pageContent?.section_assigned_ids?.find(
    (section) => section.section?.section_name === 'Planning Department'
  )?.section;

  const departmentSection2 = pageContent?.section_assigned_ids?.find(
    (section) => section.section?.section_name === 'Planning Department2'
  )?.section;
...
export default DepartmentPlanningPolicyPage;