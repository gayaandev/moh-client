...
const DepartmentMentalHealthPage = () => {
  ...
  useEffect(() => {
    const fetchPageContent = async () => {
      try {
        const response = await fetch(GET_PAGE_BY_SLUG_PUBLI_URL('mental-health-department'), {
  ...
  if (loading) {
    return (
      <PublicLayout>
        <PageHeader pageName="Mental Health Department" />
  ...
  const departmentSection1 = pageContent?.section_assigned_ids?.find(
    (section) => section.section?.section_type === 'Mental-Health-Department'
  )?.section;

  const departmentSection2 = pageContent?.section_assigned_ids?.find(
    (section) => section.section?.section_type === 'Mental-Health-Department2'
  )?.section;
...
export default DepartmentMentalHealthPage;