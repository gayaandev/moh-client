...
const DepartmentMedicalServicePage = () => {
  ...
  useEffect(() => {
    const fetchPageContent = async () => {
      try {
        const response = await fetch(GET_PAGE_BY_SLUG_PUBLI_URL('department-medical-service'), {
  ...
  const departmentSection1 = pageContent?.section_assigned_ids?.find(
    (section) => section.section?.section_type === 'Department Medical Service'
  )?.section;

  const departmentSection2 = pageContent?.section_assigned_ids?.find(
    (section) => section.section?.section_type === 'Department Medical Service2'
  )?.section;
...
export default DepartmentMedicalServicePage;